...
public class LaTeXAtom extends Atom {
	...
	boolean bool = env.getTeXFont().getBold();
	env.getTeXFont().setBold(false);
	RowAtom rat = (RowAtom)new TeXFormula("\\mathrm{XETL}").root;
	...
	CharBox A = new CharBox(env.getTeXFont().getChar('A', "mathrm", env.supStyle().getStyle()));
	A.setShift(-f);
	hb.add(A);
