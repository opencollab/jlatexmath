...
public class ArrayAtom extends Atom {
    ...
    public static SpaceAtom hsep = new SpaceAtom(TeXLength.Unit.EM, 1., 0.0, 0.0);
    public static SpaceAtom semihsep = new SpaceAtom(TeXLength.Unit.EM, 0.5, 0.0, 0.0);
    public static SpaceAtom vsep_in = new SpaceAtom(TeXLength.Unit.EX, 0.0, 1., 0.0);
    public static SpaceAtom vsep_ext_top = new SpaceAtom(TeXLength.Unit.EX, 0.0, 0.4, 0.0);
    public static SpaceAtom vsep_ext_bot = new SpaceAtom(TeXLength.Unit.EX, 0.0, 0.4, 0.0);
    ...
    /*public Box createBox(TeXEnvironment env) {
        ...
        final Box[][] boxarr = new Box[row][2 * col + 1];
        ...
        for (int i = 0; i < row; ++i) {
            ...
            for (int j = 0; j < 2 * col + 1; ++j) {
                ...
                if (at == null) {
                    boxarr[i][j] = StrutBox.getEmpty();
                } else {
                    final Box b = at.createBox(env);
                    boxarr[i][j] = b;
        ...
        for (final MulticolumnAtom multi : listMulti) {
            ...
            final int N = c + n - 1;
            double w = colWidth[c] + hseps[2 * c + 1] + vlines.get(c + 1).getWidth();
            for (int j = c + 1; j < N; ++j) {
                w += colWidth[j] + hseps[2 * j] + hseps[2 * j + 1] + vlines.get(j + 1).getWidth();
            }
        ...
        for (int j = 0; j < col; ++j) {
            matW += colWidth[j] + hseps[2 * j] + hseps[2 * j + 1] + vlines.get(j).getWidth();
        }
            ...
            for (int j = 0; j < col; ++j) {
                ...
                if (typ == TeXConstants.TYPE_HLINE) {
                    ...
                    if (typ == TeXConstants.TYPE_MULTICOLUMN) {
                        ...
                        final List<Box> mcVlines = matom.getOptions().getVlines(env);
                        if (j == 0) {
                            addVline(hb, mcVlines, 0, rhi + rdi + vsepH, rdi + halfVsepH);
                        }

                        CellBox cb = new CellBox(boxarr[i][j], rhi + halfVsepH, rdi + halfVsepH, l, r, matom.getWidth(), matom.getOptions().getAlignment(0));
                        cb.setBg(matrix.getColor(i, j));
                        hb.add(cb);
                        addVline(hb, mcVlines, 1, rhi + rdi + vsepH, rdi + halfVsepH);
                        j += n - 1;
                    } else {
                        if (j == 0) {
                        ...
                        CellBox cb = new CellBox(boxarr[i][j], rhi + halfVsepH, rdi + halfVsepH, l, r, colWidth[j], options.getAlignment(j));
                        cb.setBg(matrix.getColor(i, j));
                        hb.add(cb);
    ...
    private static void addVline(final HorizontalBox hb, final List<Box> vlines, final int i, final double h, final double s) {
