...
public class TeXParser {
    ...
    private static final int[] HEX_ARRAY = {16, 16, 16, 16, 16, 16, 16, 16, 16, 16, // 00
    ...
    public static RowAtom getAtomForLatinStr(final String s, final RowAtom ra, final boolean mathMode) {
        final ExternalFontManager.FontSSSF f = ExternalFontManager.get().getFont(Character.UnicodeBlock.BASIC_LATIN);
        if (f != null) {
    ...
    public static Atom getAtomForLatinStr(final String s, final int textStyle, final boolean math) {
    ...
    public TeXLength[] getDimensions() {
        // we are looking for keywords: width, height, depth followed by a length
        TeXLength w = null;
        TeXLength h = null;
        TeXLength d = null;
        skipPureWhites();
    ...
    private boolean convSup(final char c, final RowAtom ra) {
        ...
        case '\"':
            ...
            ra.add(Symbols.PRIME);
            break;
        case CharMapping.PRIME:
            ra.add(Symbols.PRIME);
            break;
        case CharMapping.BACKPRIME:
            ra.add(Symbols.BACKPRIME);
            break;
        case CharMapping.DEGREE:
            ra.add(Symbols.CIRC);
            break;
                ...
                while (pos < len) {
                    ...
                    if (c == ',' || c == ';') {
                        ...
                        case 1:
                            ...
                            // 123 => 3 + 2 * 16 + 1 * 16^2
                            return new Color(convertIntToHex(n.i));
                        case 3:
                ...
                if (n == 16) {
                    if (c == stop) {
                        if (ncomp == 3) {
                            ...
                            return new Color((R0G0B << 4) | R0G0B);
                        } else if (ncomp == 4) {
                            ++pos;
    ...
    /** Convert an integer like 1234 into 0x1234
        0x1234 = 4 + 3*16 + 2 * 16^2 + 1 * 16^3
     */
            ...
            if (ac instanceof GroupConsumer) {
                ...
                if (stack.isEmpty()) {
                    ...
                    if (nsize > size) {
                        ...
                        // we remove the added elements and the top consumer since
            ...
            case 'p':
                ...
                if (cn == 'i') {
                    if (pos + 1 < len) {
                        ...
                        if (cn == 'c'
                                && parseString.charAt(pos + 1) == 'a') { // pica
                            pos += 2;
                            return new TeXLength(TeXLength.Unit.PICA, x);
                        } else if (cn == 'x') { // pixel
                            if (pos + 2 < len
        ...
        if (pos < len) {
            ...
            if (c >= '0' && c <= '9') {
                ...
                while (pos < len) {
                    c = parseString.charAt(pos);
                    if (c >= '0' && c <= '9') {
                        intPart = 10 * intPart + (int)(c - '0');
                    } else {
                        if (c == '.') {
                            ++pos;
                            double x = (double)intPart + getDecimalPart();
                            if (pos < len) {
                                c = parseString.charAt(pos);
                                if (c == '%') {
                                    ++pos;
                            ...
                            ++pos;
                            final double x = ((double)intPart) / 100.;
                            return new DoubleOrInt(negative ? -x : x);
                        } else {
                ...
                for (int i = 0; i < resLen; ++i) {
                    ...
                    if (pos < len) {
                        ...
                        if (c == '}') {
                            ...
                            throw new ParseException(this, "Invalid character \'" + c + "\' in list of numbers: expect a \',\' or \';\'");
                        }
                    }
                }
                throw new ParseException(this, "Expect a \'}\'");
        ...
        if (pos < len) {
            ...
            if (c >= '0' && c <= '9') {
                ...
                while (pos < len) {
                    ...
                    if (c >= '0' && c <= '9') {
                        ++ndec;
                        decPart = 10 * decPart + (int)(c - '0');
                        ++pos;
                        if (ndec == MAX_DEC) {
                            skipNumbers();
                            return (double)decPart / POWTEN[MAX_DEC];
                        }
                    } else {
                        break;
                    }
                }
                return (double)decPart / POWTEN[ndec];
            }
        }
        return 0.;
    }
    ...
    public void convertCharacter(char c, final boolean oneChar) throws ParseException {
        if (!charMapping.replace(c, this)) {
            ...
            if (charMapping.replace(c, this)) {
                // When we got the unicodebock, an alphabet has maybe been mapped
                return;
            }
            String r;
            if (oneChar) {
    ...
    private void parseArrayOptions(final ArrayOptions options) {
        ...
        while (pos < len) {
            ...
            case '@' :
                ...
                addConsumer(cons);
                addString(code,
                          true /* to come back here after the code has been parsed */);
                parse();
