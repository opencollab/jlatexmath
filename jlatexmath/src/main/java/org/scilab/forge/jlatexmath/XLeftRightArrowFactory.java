...
public class XLeftRightArrowFactory {
    ...
    public static Box create(boolean left, TeXEnvironment env, double width) {
        ...
        if (left) {
            abody.transform(AffineTransform.getTranslateInstance(0.910 * arrw, -0.230 * arrw));
            aarr.add(abody);
        } else {
            aarr.transform(AffineTransform.getTranslateInstance(awidth - 0.090 * arrw, 0.));
    ...
    public static Box create(TeXEnvironment env, double width) {
        ...
        if (width <= TeXFormula.PREC + bodyW) {
            final double diff = Math.min(bodyW - width, 0.769 /* true body width */ * arrw);
            aright.transform(AffineTransform.getTranslateInstance(0.820 * arrw - diff, 0.));
            aleft.add(aright);
        ...
        abody.transform(AffineTransform.getTranslateInstance(0.910 * arrw, -0.230 * arrw));
        aleft.add(abody);
        aright.transform(AffineTransform.getTranslateInstance(0.820 * arrw + awidth, 0.));
        aleft.add(aright);
    ...
    public static Box createXMapsto(TeXEnvironment env, double width) {
        ...
        final Area aright = right.getArea();
        if (width <= TeXFormula.PREC + bodyW) {
            final double diff = Math.min(bodyW - width, 0. /* mapsto width */ * arrw);
            aright.transform(AffineTransform.getTranslateInstance(0. * arrw - diff, 0.));
            aleft.add(aright);
            return new ShapeBox(aleft);
        }
        ...
        final HorizontalRule body = new HorizontalRule(drt, awidth, 0.);
        final Area abody = body.getArea();
        abody.transform(AffineTransform.getTranslateInstance(0.096 * arrw, -0.230 * arrw));
        aleft.add(abody);
        aright.transform(AffineTransform.getTranslateInstance(0. * arrw + awidth, 0.));
        aleft.add(aright);
    ...
    private static Box createExtension(TeXEnvironment env,
