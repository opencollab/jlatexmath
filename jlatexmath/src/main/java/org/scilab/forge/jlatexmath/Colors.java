...
public class Colors {
	...
	public final static int clamp(final int n) {
		return Math.min(255, Math.max(n, 0));
	}

	public final static double clamp(final double x) {
		return Math.min(1., Math.max(x, 0.));
	}

	public final static float clamp(final float x) {
		return Math.min(1f, Math.max(x, 0f));
	}
	...
	public static Color convHSB(double h, double s, double b) {
		h = normH(h);
		s = Colors.clamp(s);
		b = Colors.clamp(b);

		if (s == 0f) {
			return new Color((float) b, (float) b, (float) b, 0f);
		}

		final int i = (int) (6. * h);
		final double f = 6. * h - i;
		final double p = b * (1. - s);
		final double q = b * (1. - s * f);
		final double t = b * (1. - s * (1. - f));
		switch (i) {
		case 0:
			return new Color((float) b, (float) t, (float) p, 0f);
		case 1:
			return new Color((float) q, (float) b, (float) p, 0f);
		case 2:
			return new Color((float) p, (float) b, (float) t, 0f);
		case 3:
			return new Color((float) p, (float) q, (float) b, 0f);
		case 4:
			return new Color((float) t, (float) p, (float) b, 0f);
		case 5:
			return new Color((float) b, (float) p, (float) q, 0f);
		default:
			return new Color(0); // Impossible
		}
	}
